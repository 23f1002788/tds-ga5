...
def _choose_effect(root_cause: str, effect_tools: List[str], approval_tools: set,
    ...
    want = canonical_effects.get(root_cause)
...
def _handle_outcomes(state: Dict[str, Any], receipt_id: str,
    ...
    for oc in outcomes or []:
        ...
        if status == 200 or rclass == "success":
            act["confirmed"] = True
            act["resolved"] = True
        elif att["attempt"] < 2 and (err in ("503", "timeout") or status in (503, 504)):
            next_attempt = att["attempt"] + 1
            phase = "diagnostic" if act in state["diagnostics"] else "effect"
            extra = {}
            if phase == "effect" and act.get("needs_approval"):
                extra = {"approvalId": act.get("approvalId"), "approvalNonce": act.get("approvalNonce")}
            disp = new_dispatch(state, act, next_attempt, phase, extra)
            retry_dispatches.append(disp)
        else:
            act["failed"] = True
            act["resolved"] = True

    if state["phase"] == "await_diag":
        if _all_diag_resolved(state):
            resp = _advance_after_diagnostics(state)
            state["last_response"] = resp
            return resp
...
def _handle_approvals(state: Dict[str, Any], receipt_id: str,
                      approvals: List[Dict[str, Any]]) -> Dict[str, Any]:
    eff = state.get("effect")
    if not eff or not eff.get("needs_approval"):
        raise HTTPException(status_code=422, detail="No pending approval required")

    appr_match = None
    for app in approvals or []:
        if app.get("approvalId") == eff.get("approvalId"):
            appr_match = app
            break

    if not appr_match:
        raise HTTPException(status_code=422, detail="Approval ID mismatch")

    decision = str(appr_match.get("decision", "")).lower()
    nonce = appr_match.get("nonce")
    ...
    if decision in ("approved", "grant", "allow"):
        eff["approved"] = True
